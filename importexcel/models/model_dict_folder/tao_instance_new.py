...
from odoo.addons.importexcel.models.model_dict_folder.recursive_func import muon_xuat_dac_tinh_gi,ordered_a_model_dict,recursive_add_model_name_to_field_attr,define_col_index,define_col_index,check_xem_att_co_nam_ngoai_khong,write_get_or_create_title,check_col_index_match_xl_title,rut_gon_key
...
#     if 'cach_tim_location_goc' not in gen_model_dict_kargs:
    ...
    if callable(CHOOSED_MODEL_DICT):
        CHOOSED_MODEL_DICT = CHOOSED_MODEL_DICT(self=self, key_tram=self_key_tram, gen_model_dict_kargs=gen_model_dict_kargs)
    if not self.file:
    ...
    #R2
    rut_gon_key(CHOOSED_MODEL_DICT,key_tram)
...
#             rut_gon_key(COPY_MODEL_DICT,key_tram)
        ...
        set_is_largest_map_row_choosing = get_key(CHOOSED_MODEL_DICT, 'set_is_largest_map_row_choosing')#set_is_largest_map_row_choosing  is boolean
...
#         print ('***COPY_MODEL_DICT',COPY_MODEL_DICT)
        ...
        for number_row_count,row in enumerate(range(first_row, last_row)):
            ...
            create_instance (self, COPY_MODEL_DICT, 
                                              sheet, 
                                              row, 
                                              merge_tuple_list, 
                                              needdata, 
