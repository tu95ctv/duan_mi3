...
#     diem_tvcv = fields.Float(digits=(6,2),string=u'Điểm Thư Viện',compute='diem_tvcv_',store=True,readonly=True)# 
...
#     @api.depends('diem_tvcv','so_luong','so_lan','len_gd_child')
#     @skip_depends_if_not_congviec_decorator
...
#     @api.depends('so_luong','so_lan', 'diem_tvcv','gd_parent_id.diem_remain_gd','loai_record')
...
#                         raise ValidationError(u'Có Hưởng điểm con Trùng: user_ids %s- set(user_ids) %s'%(user_ids, set(user_ids)))                        
    ...
    def gen_vals_for_cd_children_ids(self,vals_2,vals,fields_not_copy=['cd_children_ids','ti_le_chia_diem','user_id' ]):
        if not vals_2:
            vals_2 = copy(vals)
...
#                 raise UserError('cd_children_ids %s'%cd_children_ids)
                adding_cd_children_ids = list(filter(lambda i:i[0]==0 or i[0]==2, cd_children_ids)) # thêm hoặc xóa đi cvi chia điểm
            ...
            if r.cd_children_ids:
                vals_2 = self.gen_vals_for_cd_children_ids(vals_2,vals)
                r.cd_children_ids.write(vals_2)
            if r.hd_children_ids:
                vals_2 = self.gen_vals_for_cd_children_ids(vals_2,vals, ['hd_children_ids'])
